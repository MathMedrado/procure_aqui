...
class _UserProfileExclusionState extends State<UserProfileExclusion> {
  ...
  final snackBar = SnackBar(content: Text('O password ou email informado é inválido!', textAlign: TextAlign.center), backgroundColor: Colors.red,);
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    if(_formKey.currentState!.validate()) {
                      ...
                      bool value = true;
                      if(value == true){
                          var url = Uri.parse('http://10.0.2.2:8000/validateUser/${widget.user.getId}/email/${emailController.text}/password/${passwordController.text}');
                          var response = await http.get(url);
                          print(url);
                          print(response.body);
                          var values = jsonDecode(response.body);
                          var userExist = values['is_product'];
                          print(userExist);
                          if(userExist){
